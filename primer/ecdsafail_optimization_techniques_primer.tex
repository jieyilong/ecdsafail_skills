...
--- keeping all \textbf{512} of its qubits off the GCD peak. This is the decisive product-min lever of
the ludicrous design (it broke the 1168q wall; \S\ref{sec:qclassical} develops it as a technique).
...
\begin{itemize}
  ...
  \item \textbf{Solinas prime} (a.k.a.\ \emph{generalized Mersenne}, Jerome Solinas, 1999) --- $c$ is
    ...
    $c = 2^{32} + 977 = 2^{32} + 2^{10} - 2^{6} + 2^{4} + 1$ (see NAF, \S9.7) --- only $\sim$5 signed
...
\textbf{A $-512$q lever \emph{within the ludicrous design} --- and the move that broke the 1168q wall.}
The second EC point $Q = (Q_x, Q_y)$ is a \emph{fixed, compile-time-known constant} (the generator).
Naively you would load its two coordinates into 256-bit quantum registers and let them ride through
the point-add. But two 256-bit quantum registers that sit \textbf{live across the GCD peak} cost
\textbf{512 qubits at the binder} --- the most expensive place to spend them. A ludicrous circuit that
held $Q$ resident would be $\sim$512 qubits \emph{higher}; keeping $Q$ classical and off-peak is what
makes ludicrous's operating point viable at all.

A caveat on the ``512'': that is the lever's saving \emph{relative to the same circuit holding $Q$
quantum} --- \textbf{not} a 512-qubit leaderboard drop. On the board it showed only as
1168$\to$1167 ($-1$q): ludicrous is a fresh \emph{reimplementation} of the dialog-GCD inversion (not a
new engine), with its own cost structure. Its real impact was \textbf{breaking the 1168q wall} and
opening the headroom that then drove to 1152 (\S\ref{sec:history}, Era 3) --- the lever is decisive
for the \emph{reimplementation}, not a one-shot $-512$ on the record.
...
\textbf{Why the digit count of a constant matters.} A Solinas fold (\S\ref{sec:pmarith}) reduces the
high part $H$ of a wide value by adding $H \cdot c$ back into the low bits, where $c = 2^{32} + 977$.
Because $c$ is a \emph{sum of powers of two}, you never run a general multiplier --- $H \cdot c$ is
just \textbf{one shifted addition of $H$ per non-zero bit of $c$}. So the number of modular adds per
fold \emph{equals the number of non-zero digits of $c$}. Fewer digits $\Rightarrow$ fewer adds
$\Rightarrow$ fewer Toffoli, directly.

\textbf{Standard binary is wasteful when a constant has a run of 1-bits.} Write 977 in binary:
\[
  977 = 1111010001_2 = 2^9 + 2^8 + 2^7 + 2^6 + 2^4 + 2^0 \quad\text{(six 1-bits $\to$ six shifted adds)}.
\]
The four \emph{consecutive} 1-bits at positions 6--9 are the waste: each costs a separate add.

\textbf{Non-Adjacent Form (NAF)} allows each digit to be \textbf{signed} ($-1$, $0$, or $+1$ instead
of just $0/1$) and picks the representation with the \emph{fewest} non-zero digits (and, as the name
says, never two non-zero digits adjacent). The trick it exploits: a \textbf{run of 1-bits collapses
into a single subtraction}, using $1 + 2 + 4 + \dots + 2^{k-1} = 2^k - 1$ shifted up:
\[
  2^6 + 2^7 + 2^8 + 2^9 = 2^{10} - 2^6 \quad\text{(four terms become two)}.
\]
Applying that to 977: its 1-bits sit at positions $\{0,4,6,7,8,9\}$, and only the \emph{run}
$\{6,7,8,9\}$ collapses (to $+2^{10}-2^6$); the lone bits at 4 and 0 are already isolated, so they
carry over unchanged:
\[
  \underbrace{2^9 + 2^8 + 2^7 + 2^6}_{\text{the run}} + 2^4 + 2^0
  \;=\; \underbrace{2^{10} - 2^6}_{\text{collapsed}} + 2^4 + 2^0 .
\]
So $\mathbf{NAF(977) = +2^{10} - 2^6 + 2^4 + 2^0}$ (check: $1024 - 64 + 16 + 1 = 977$).

\textbf{Why ``$+,-,+,+$'' alone can't tell you the powers --- and isn't supposed to.} A NAF number is
\textbf{positional}, exactly like ordinary binary except each position may hold $-1$ as well as $0$ or
$+1$. So the real object is a digit \emph{string}, one digit per bit position:
\begin{center}
...
\end{tabular}
\end{center}
The signs ``$+,-,+,+$'' are just the four \textbf{non-zero} digits read off top-to-bottom --- a
summary, not the representation. The powers $10, 6, 4, 0$ are simply \emph{where those non-zero digits
sit}, intrinsic to the number, not inferred from the signs. ``Signs $+,-,+,+$ on $9,8,5,1$'' would be
a \textbf{different digit string} --- a different number entirely (it sums to $512 - 256 + 32 + 2 =
290$, not 977). Which positions are non-zero is decided when you \emph{compute} the NAF (the
run-collapse above, or the standard greedy algorithm), and the value 977 has exactly \textbf{one} NAF
--- so there is nothing to ``choose.''

A $-2^k$ digit means \emph{that} fold step is a \textbf{shifted subtraction} instead of an addition ---
and in a reversible circuit a controlled modular subtract costs exactly the same as a controlled add
(it is the adder run backwards), so a signed digit is as cheap as a binary one.

\textbf{It is baked in at compile time.} $977$ (and $c = 2^{32} + 977$) is the secp256k1 prime --- a
\emph{fixed constant known when the circuit is built}. So its NAF is computed \textbf{once at build
time}, and those positions become a \textbf{fixed gate sequence}: every Solinas fold emits exactly the
same four shifted operations ($+2^{10}$, $-2^6$, $+2^4$, $+2^0$). Nothing is stored in qubits or
decided at runtime, and it is not recomputed per input or per GCD step --- it is \emph{structure}.
This is the constant-folding fact of \S9.9: arithmetic against a \emph{known} constant compiles to a
fixed pattern of gates. (Were the multiplier a \emph{quantum} value instead, you couldn't bake in a
NAF --- you'd need per-bit controlled operations or windowed table lookups.)

\textbf{The payoff.} 6 non-zero digits $\to$ 4: \textbf{four shifted add/subtracts per fold instead of
six, $\approx33\%$ off the 977 part of every Solinas reduction.} That reduction runs on essentially
every modular multiply/square across all $\sim$530 GCD steps, so the saving compounds. The same
recoding is applied to the full $c = 2^{32} + 977$ and to the $2^{32}$ term of the pseudo-Mersenne
fold (\S\ref{sec:pmarith}).

\textbf{Density-neutral (\S\ref{sec:density})}: $2^{10} - 2^6 + 2^4 + 1 = 977$ is a mathematical
identity, correct for all inputs, so it stacks freely with no nonce hunt. (Any minimal-weight signed
recoding works; the codebase sometimes writes an equivalent 4-term form like $2^{10} - 2^5 - 2^4 + 1$
--- note that one is not \emph{strictly} NAF, since $2^5$ and $2^4$ are adjacent, but it has the same
4-vs-6 digit count and the same cost.)
...
\subsubsection{Era 1 --- Pre-history: 2715q $\to$ 2002q (the Roetteler/Kaliski era)}

The challenge \emph{started} at \textbf{2{,}715 qubits $\times$ $\sim$3.96M Toffoli}: a
\textbf{Roetteler-style two-Kaliski affine} point addition --- affine coordinates (not projective),
the modular inverse done by \emph{two} runs of the \textbf{Kaliski binary-GCD} algorithm,
direct/Solinas reduction (no Montgomery), with the prime and $Q$ known as compile-time constants. The
inverse was $\sim$80--90\% of the Toffoli (the bottleneck \S\ref{sec:circuit} names), treated as
\textbf{algorithmically fixed} --- so for this whole era the gains came not from a better inverse but
from packing scratch ever tighter around it. These are the same techniques in this primer, in their
first and cruder form.

\smallskip\noindent\textbf{Early Toffoli wins (still at 2708q):}
\begin{itemize}
  \item \textbf{cswap boundary-merge} (\S\ref{sec:cswapmerge}): merge the B\'ezout \texttt{cswap}s that
    straddle adjacent Kaliski iterations via $\texttt{cswap}(p)\cdot\texttt{cswap}(q)=\texttt{cswap}(p
    \oplus q)$, carried on one frame-parity qubit --- \textbf{$-274$k Toffoli}, peak-neutral. The era's
    single biggest gate win.
  \item \textbf{Solinas-skip on early doublings}: in the early iterations the operands satisfy
    $\max(r,s) \le 2^{\text{iter}}$, so a \texttt{mod\_double}'s Solinas constant-add is the
    \emph{identity} --- replace it with a plain wire-relabel shift (0 Toffoli).
  \item \textbf{Late-iteration carry recovery}: the Kaliski STEP-4 adds borrow their carry register
    from bits that are \textbf{provably \ket{0} as a classical function of the iteration index} (the
    high bits of $u$, since $\mathrm{bitlen}(u) \le 2n-\text{iter}$) --- clean for \emph{all} inputs
    (gate-hosting \S\ref{sec:gatehost} / borrowed-carry \S\ref{sec:borrowcarry} in their first form).
\end{itemize}

\smallskip\noindent\textbf{The qubit drops 2708 $\to$ 2002 (scratch-packing):}
\begin{itemize}
  \item \textbf{Joint-pin plateau breaking} (\S7.9) --- the recurring method, present from the very
    start: a peak co-owned by several multiply-scratch clusters drops only when you reduce \emph{all of
    them together}. This one insight drives every qubit drop in the era.
  \item \textbf{Square-recompute eviction} (\S\ref{sec:liverange}): uncompute the dead 512-bit
    $\lambda^2$ product before the affine $y$-multiply, run the multiply without it resident, recompute
    once after --- a live-range hole worth $\sim$512 qubits at the binder.
  \item \textbf{Dialog history-fold}: relabel the per-iteration Kaliski history qubits into the
    \textbf{idle, provably-\ket{0} high bits of the shrinking GCD register} instead of a dedicated bank
    (0-Toffoli qubit relabeling --- the shrinking-register idea that dynamic-width \S7.6 later
    formalizes).
  \item \textbf{Carry-pool relocation}: move the wide STEP-4 add/sub carries off their dedicated
    $\sim$277-qubit pool onto provably-zero high bits of the operands, freeing the history pool.
    Together these reach the \textbf{published ``$9n$'' peak floor}.
\end{itemize}

By $\sim$2002q the Kaliski route was packed about as tight as it goes: with the inverse held fixed,
scratch-packing had run its course. The next jump needed a \emph{new engine} --- which opens Era 2.

\subsubsection{Era 2 --- The dialog-GCD frontier: 2002q $\to$ 1168q (engine swap, then the wall)}

\textbf{The engine swap (2002 $\to$ 1698)} is the first true \emph{architectural} jump: it replaced
the entire Kaliski stack with a \textbf{dialog-GCD inverter} --- one that \textbf{streams a compressed
transcript} of its branch decisions (\S\ref{sec:dialog}) instead of holding wide history registers
resident. A single change worth \textbf{$-304$ qubits}, and the lineage everything after descends
from. On this new engine the peak then fell steadily via the \textbf{windowed/chunked apply} dial
...
keep-live-vs-recompute in the Solinas fold (\S\ref{sec:liverange}), the first
\textbf{recompute-to-free fold carries}, and progressively tighter transcript codecs:
...
\midrule
2002$\to$1698 ($-304$q) & \textbf{Dialog-GCD engine swap} (\S\ref{sec:dialog}) & replaced the Kaliski stack with a streamed compressed transcript \\
...
\textbf{``ludicrous''} point-add onto the challenge. It is \emph{not} a new inversion algorithm --- it
still uses the \textbf{dialog-GCD record/replay} engine (here in its \textbf{jump-2 divstep} variant,
\S\ref{sec:dialog}) --- but a \textbf{fresh reimplementation} that re-architects the circuit around
several techniques the in-progress dialog-GCD codebase lacked, \textbf{above all keeping $Q$ off the
peak (\S\ref{sec:qclassical})}. It landed at \textbf{1167q $\times$ 1{,}422{,}591} (only $-1$q below
the wall on day one), but the point wasn't the $-1$q: the reimplementation had \textbf{headroom the
stalled dialog-GCD line had run out of}, and over the next days drove to 1152. Its design (the
architecture \S\ref{sec:trailmix} describes in full):
...
Classical constants off-peak (\S7.20) & 512 \emph{within ludicrous} (broke the 1168 wall) & 0 & Yes \\
