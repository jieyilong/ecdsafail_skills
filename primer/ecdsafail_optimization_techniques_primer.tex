...
\[
  \mathrm{NAF}(977) = 2^{10} - 2^6 + 2^4 + 2^0 \quad\text{(four signed terms: } +,\,-,\,+,\,+\text{)}
\]
(check: $1024 - 64 + 16 + 1 = 977$). The $-2^6$ digit means \emph{that} fold step is a \textbf{shifted
subtraction} instead of an addition --- and in a reversible circuit a controlled modular subtract
costs exactly the same as a controlled add (it is the adder run backwards), so a signed digit is as
cheap as a binary one.
