...
\]
So $\mathbf{NAF(977) = +2^{10} - 2^6 + 2^4 + 2^0}$ (check: $1024 - 64 + 16 + 1 = 977$). Note the
exponents $10, 6, 4, 0$ are \emph{not} a free choice and each sign is glued to its own power: the
run-collapse \emph{produces} $+2^{10}$ and $-2^6$, and the untouched bits stay at $+2^4$ and $+2^0$.
(That is why it is not, say, ``signs $+,-,+,+$ on $9,8,5,1$'' --- those powers don't come out of the
derivation and wouldn't sum to 977.) A $-2^k$ digit simply means \emph{that} fold step is a
\textbf{shifted subtraction} instead of an addition --- and in a reversible circuit a controlled
modular subtract costs exactly the same as a controlled add (it is the adder run backwards), so a
signed digit is as cheap as a binary one.
