...
A caveat on the ``512'': that is the lever's saving \emph{relative to a same-family circuit holding
$Q$ quantum} --- \textbf{not} a 512-qubit leaderboard drop. On the board it showed only as
1168$\to$1167 ($-1$q), because ludicrous is a different family with its own cost structure. Its real
impact was \textbf{breaking the 1168q wall} and opening the line that then drove to 1152
(\S\ref{sec:history}, Era 3) --- the lever is decisive for the \emph{family}, not a one-shot $-512$ on
the record.
