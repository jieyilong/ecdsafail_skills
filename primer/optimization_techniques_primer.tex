...
This primer is written for an undergrad who has just learned the basics of quantum computing ---
superposition, entanglement, Toffoli/CNOT/Hadamard gates, quantum circuits, and reversible
computation. It explains every optimization technique used in the ecdsa.fail challenge from
circuit-level first principles, with concrete examples from actual research.
\end{tcolorbox}
\vfill
...
% ===========================================================================
\section{Qubit Reduction Techniques (1--19)}
\label{sec:qubittech}
% ===========================================================================
...
\subsubsection{Pre-history: 2715q $\to$ 1211q (where the circuit came from)}

The challenge \emph{started} at a baseline of \textbf{2{,}715 qubits $\times$ $\sim$3.96M Toffoli}.
The journey down to the 1211q dialog-GCD circuit (where the table below begins) is its own story, and
it shows the same methods appearing in cruder form:

\begin{itemize}
  \item \textbf{The baseline} was a \textbf{Roetteler-style two-Kaliski affine} point addition: affine
    coordinates (not projective), the modular inverse done by \emph{two} runs of the Kaliski
    binary-GCD algorithm, with $Q$ and the prime kept \textbf{classical from the start} and
    direct/Solinas reduction (no Montgomery). The inverse was $\sim$80--90\% of the Toffoli --- the
    bottleneck \S\ref{sec:circuit} names.
  \item \textbf{2708 $\to$ $\sim$2002 (scratch-packing on a fixed inverse).} With the Kaliski engine
    fixed, the early drops were \emph{all} live-range and hosting tricks: the $\texttt{cswap}(p\oplus
    q)$ swap-merge (\S\ref{sec:cswapmerge}, $-274$k Toffoli), borrowing \textbf{provably-\ket{0} high
    bits} of the shrinking GCD register as carry scratch (zero as a classical function of the
    iteration index --- gate-hosting \S\ref{sec:gatehost} in its first form, reaching the published
    ``$9n$'' peak floor), square-recompute eviction (\S\ref{sec:liverange}), and \textbf{joint-pin
    plateau breaking} (\S7.9) --- the realization, present from the very start, that a
    peak co-owned by several scratch clusters drops only when you reduce them all \emph{together}.
  \item \textbf{2002 $\to$ 1698 (the engine swap).} The first true \emph{architectural} jump replaced
    ...
    instead of holding wide history registers resident (\S\ref{sec:dialog}) --- one change worth
    $-304$ qubits, and the lineage the SOTA still descends from.
  \item \textbf{1698 $\to$ 1211 (windowing + recompute-to-free).} On the dialog-GCD engine the peak
    fell via the \textbf{windowed/chunked apply} dial (\S\ref{sec:windowed}, the era's biggest peak
    lever), carry self-hosting on the operand lanes (\S\ref{sec:borrowcarry}/\S\ref{sec:gatehost}),
    shift-free modular doublings (\S\ref{sec:shiftfree}), keep-live-vs-recompute in the Solinas fold
    (the keep-alive dual, \S\ref{sec:liverange}), and the first \textbf{recompute-to-free fold
    carries} --- the same family as the cy0 trick (\S7.5) that later broke
    1153$\to$1152.
\end{itemize}

So nearly every technique in this primer was \emph{already in play} before 1211q, just on a coarser
circuit. The table below picks up at 1211q, where the modern dialog-GCD/trailmix line begins.

\subsubsection{The 1211 $\to$ 1152 ladder}
