...
\addcontentsline{toc}{section}{Introduction: What Is the ecdsa.fail Challenge?}
...
\begin{enumerate}
  ...
  \item \textbf{Explore the (Q, T) Pareto frontier} --- especially the \emph{useful corner} where
    ...
    \emph{clean, reusable basis curve}: a value-exact circuit at each qubit count that others can fork
    and improve (see the 1153q$\to$1133q clean Pareto bases, \S\ref{sec:paretopush}). This track
    deliberately avoids island-overfit tricks (\S\ref{sec:density}) so the curve stays a sound
    reference.
\end{enumerate}
...
compression (\S6.8) and the vented adders (\S8.1) are aimed. And one clean knob sits behind the whole
...
(mod-double, mod-add, the $+f$ fold); \S8.7 (NAF) makes the fold constant itself cheaper.
...
% ===========================================================================
\section{Qubit Reduction Techniques (1--17)}
\label{sec:qubittech}
% ===========================================================================
...
% ===========================================================================
\section{Toffoli Reduction Techniques (1--18)}
\label{sec:toftech}
% ===========================================================================
...
keep plain integer representation and fold directly. NAF recoding (\S8.7) is the next layer: it makes
...
\midrule
Live-range hole (\S6.1) & remove a pebble by \textbf{uncompute}, re-place later by \textbf{recompute} & Bennett & full Toffoli, \emph{twice} \\
Free-and-recompute cy0 (\S6.5) & same, on one cheap pebble & Bennett & $\approx$0 (a few CNOTs) \\
MBU / carry vent (\S8.1--B) & remove a carry pebble by \textbf{X-measurement $+$ CZ} & spooky & \textbf{0 Toffoli} \\
Conditional replay (\S8.3) & measure the pebble; recompute only on the firing branch & spooky & $\Pr[\text{fire}] \times$ cost \\
HMR ghosting (\S6.14) & measure, leave a \textbf{ghost}, recompute $+$ phase-fix & spooky & one recompute \\
Transcript compression (\S6.8) & \textbf{don't pebble it} --- shrink the data so fewer pebbles persist & (out of the game) & codec only \\
...
hole (\S6.1) is a \emph{Bennett} move: to free a pebble you uncompute it (paying its Toffoli) and
later recompute it (paying again) --- the classical-reversible game, where un-pebbling is expensive.
MBU (\S8.1) is the \emph{spooky} move on the same kind of pebble: you remove it by \textbf{measuring}
...
\begin{itemize}
  \item \textbf{The transcript is firmly in the ``never recompute'' regime.} Bennett's
    ...
    the data} (\S6.8), not by recomputing it.
...
\textbf{data pebbles} --- transcript, passengers, coordinate registers --- exactly where \S6 and
...
\subsection{The score track (Q$\times$T): the 1211$\to$1152 journey}
...
1193$\to$1192 ($-1$q) & Truncation (\S6.4) & Square segment one notch tighter \\
1192$\to$1185 ($-7$q) & Transcript codec (\S\ref{sec:transcript}) & Per-step fold carry scheduling + exact 11-bit head codec \\
1185$\to$1170 ($-15$q) & Plateau decomp (\S6.9) + 5 techniques & Tail codec, streaming apply, square rebalance, 5 co-binders \\
...
Toffoli: $-22.4$M & Karatsuba square (\S8.6) & 3 sub-squares instead of 4 \\
Toffoli: $-1.33$M & GAP\_J2 narrowing (\S8.11) & 22-line per-step comparator table edit \\
Toffoli: $-377$k & Constprop deeper (\S8.16) & \texttt{CONSTPROP\_MAX\_ITERS} $16\to256$ \\
1163$\to$1159$\to$1156 & Headroom clamp returns (\S6.10 + shelved-lever \S\ref{sec:exchange}) & Same clamp that lost early now wins on the cheap Karatsuba$+$dead-CCX base ($\sim$527 T/qubit) \\
1156$\to$1153 & Clamp $+$ zero-cin fold $+$ FFG cap (\S6.10), paid by cinc (\S\ref{sec:cinc}) $+$ iterated dead-CCX (\S\ref{sec:deadccx}) & Qubit drop alone cost $+11{,}369$ avgT (rejected); Toffoli levers stacked back to win \\
\textbf{1153$\to$1152} & Free-and-recompute cy0 (\S6.5) & \texttt{cy0 = ctrl \& !a0}; loan lane for 40 binding folds \\
Structural & 6dafa07 pivot (\S8.8/\S\ref{sec:density}) & Replaced empirical dead-CCX with $\approx15$ structural predicates \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{The levers that define the track} (each is a \S6 technique, applied to the divstep machine):
\begin{itemize}
  \item \textbf{Dynamic-width registers (\S6.6)} --- resize $A,B,CA,CB$ per step; free known-zero
    high bits.
  ...
  \item \textbf{Known-constant teardown (\S6.7)} --- at convergence $A{=}0, B{=}1, CA{=}p, Q{=}0$ are
    constants; XOR them out (0 Toffoli) before the $\lambda$ multiply so they don't co-reside with it.
  \item \textbf{Passenger ghosting (\S6.14)} --- HMR-ghost $dy$/$\lambda$ so only one 256-bit
...
The third track (\S\ref{sec:twocomps}, objective 3) maps the \textbf{value-exact Pareto frontier}
below the 1153q SOTA as a sequence of \textbf{clean, dead-CCX-free basis circuits} others can fork.
...
qubits --- clamp a peak fold's measured-vent windows, \S8.2), \textbf{no-ancilla / dirty-borrow
...
\S6.1/\S\ref{sec:pebbling}), and --- new at 1133 --- \textbf{GCD active-width trimming}: per-step
shrinking of the live GCD operand width in the convergence tail
(\texttt{TLM\_GCD\_ACTIVE\_WIDTH\_TRIM=3} after step 205), the dynamic-width-register idea (\S6.6)
...
\href{https://github.com/jieyilong/ecdsafail_skills/blob/main/ecdsafail-circuit-optimization/references/pareto-frontier-push-1153-to-1133.md}{\texttt{pareto-frontier-push-1153-to-1133.md}}.
...
\begin{enumerate}
  ...
  \item \textbf{FFG cy0-style recompute on other binders}: The cy0 trick (\S6.5) worked because
...
\midrule
Live-range hole (\S6.1) & $k$ qubits & $+2 \times C_V$ & Yes \\
Passenger relocation (\S6.2) & $k$ qubits & 0 & Yes \\
Range-bound guard removal (\S6.3) & 1 per register & 0 & Yes \\
Truncation (\S6.4) & varies & 0 & Partial \\
Free-and-recompute cheap value (\S6.5) & 1 & $\approx0$ (CNOTs) & Yes \\
Dynamic-width registers (\S6.6) & varies & small & Partial \\
Known-constant teardown (\S6.7) & $k$ & 0 (CNOT $=$ Clifford) & Yes \\
Transcript compression (\S6.8) & varies & decode overhead & Yes \\
Plateau decomposition (\S6.9) & 1 (global) & combined cost & Yes \\
Dynamic headroom clamp (\S6.10) & 1 per ceiling drop & small & Yes \\
Lazy carry liveness (\S6.11) & 1/chunk & 0 & Yes \\
In-place decode (\S6.12) & varies & codec overhead & Yes \\
Borrowed-carry fusion (\S6.13) & $k$ & 0 & Yes \\
HMR ghosting (\S6.14) & 256 & $+1$ modular multiply & Yes \\
EEA register sharing (\S6.15) & $\sim$97--119 (to 851$\to$829q) & high (irrelevant to low-q objective) & Yes --- low-qubit competition \\
Gate-hosting (\S6.16) & 1 per hosted gate & 0 (clean) / $+1$ Tof (dirty) & Yes if host-zero \emph{proven}; island-exact if \emph{sampled} \\
Reset-bounded id compaction (\S6.17) & gap (max-id $-$ true peak) & 0 & Yes (relabels wires only) \\
GCD active-width trim (\S6.6 applied) & varies (deep tail) & small & Partial (graded; nonce-hunted) \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
MBU / carry vent (\S8.1/2) & 1 per vent & $+1$ temporary & Yes \\
Conditional replay (\S8.3) & $(1-f)\times\text{block}$ & 0 & Yes \\
Algebraic fusion (\S8.4) & varies & 0 & Yes \\
Witness-first dataflow (\S8.5) & $\approx13$M / instance & 0 & Yes \\
Karatsuba square (\S8.6) & $\approx22$M (one change) & 0 & Yes \\
NAF recoding (\S8.7) & $\approx5$--10\% of fold cost & 0 & Yes \\
Pseudo-Mersenne reduction (\S8.18) & huge (15\% vs 34\% of CCX) & 0 & Yes (fold); Partial (lsbs/msbs trunc) \\
Structural dead-gate skip (\S8.8) & varies & 0 & Yes \\
Classical constant ops (\S8.9) & significant & 0 & Yes \\
Converged-tail elision (\S8.10) & hundreds/iter $\times K$ & 0 & No \\
GAP\_J2 narrowing (\S8.11) & $\approx1.33$M / 22-line edit & 0 & No \\
Ancilla-free majority (\S8.12) & 1 per majority & 0 & Yes \\
Exact-adder recovery (\S8.13) & $\approx2{,}600$/call & 0 & Yes \\
Redundant cleanup skip (\S8.14) & varies & 0 & Risky \\
Off-peak loosening (\S8.15) & free refund & 0 & Yes \\
Constprop deeper (\S8.16) & $\approx377$k & 0 & Yes \\
Cinc cascade replacement (\S8.17) & $\approx5{,}175$ ($O(t^2)\to O(t)$) & 0 & Yes \\
Empirical dead-CCX drop (\S8.8) & $\approx13{,}000+$ (1st pass) & 0 & No (island-overfit) \\
Iterated 2-pass dead-CCX (\S8.8) & $\approx2{,}400$/extra pass & 0 & No (island-overfit) \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}\small
  ...
  \item \href{https://github.com/jieyilong/ecdsafail_skills/blob/main/ecdsafail-circuit-optimization/references/pareto-frontier-push-1153-to-1133.md}{\texttt{pareto-frontier-push-1153-to-1133.md}} --- the (Q,T) Pareto-frontier push (\S\ref{sec:paretopush})
