...
\subsection{EEA Register Sharing --- The Qubit Floor (and Why It Is an Anti-Lever)}
\label{sec:regshare}

\textbf{The deepest qubit-reduction idea, and the most important cautionary tale.} This is the
technique behind the lowest qubit numbers ever recorded on the challenge --- the
948q $\to$ 851q $\to$ 829q descent --- but it is also the cleanest example of a lever that
\emph{wins the qubit axis and loses the score by 250$\times$}. Understanding both halves is the
point.
...
\textbf{851-qubit peak} for the whole point-add --- a 97-qubit drop below the prior 948q record.

\textbf{Why it is an anti-lever (the catch to internalize).} A packed word has a
\emph{data-dependent boundary}: ``where does $A$ end and $CA$ begin?'' depends on values computed at
runtime. Every arithmetic step must first \textbf{locate and realign that boundary with a
full-width comparator}, and because the circuit is reversible, that comparator must be recomputed
\emph{before AND after} each hosted gate. The bookkeeping:
...
Across 530 divsteps this surrounded-comparator envelope lands at $\sim 4.6 \times 10^8$ Toffoli ---
the 851q circuit measures \textbf{464{,}495{,}956 average Toffoli, an 8.5$\times$ explosion} over
the 948q route's 54.8M. The score: $851 \times 464{,}495{,}956 \approx 3.95 \times 10^{11}$,
rejected at \textbf{$+3659\%$}, roughly 250$\times$ over the product break-even.

This is the qubit$\leftrightarrow$Toffoli-depth reciprocal (\S\ref{sec:exchange})
\emph{violated in the flesh}: the saved lanes are paid back as full-width recompute on both sides of
every gate, so the trade is \textbf{super-linear, not 1:1}. The qubit minimum and the score minimum
are simply not reached by the same construction.

\begin{tcolorbox}[noteboxstyle, title={851q is an analysis oracle, not a circuit}]
The implementing module says so itself: \lstinline{register_shared_eea.rs} is labelled
\emph{``an analysis oracle, not a reversible circuit implementation $\ldots$ passing this oracle
does not establish a challenge-valid qubit count.''} It \emph{models} the qubit count assuming the
hosting is free, then charges the hosting Toffoli separately. The subsequent 851$\to$829q race
(three competitors leapfrogging over $\sim$26 hours via ever-more-aggressive borrowing on a
\emph{frozen} width envelope) is the same disclaimed oracle, accreting turn by turn --- every rung
still \lstinline{proof_status: "reject"}.
\end{tcolorbox}

\textbf{The lesson.} Register sharing is the genuine route below $\sim$948 qubits, and the
Luo-2026 / Proos--Zalka mechanism is the structural source worth knowing. But on the \emph{product}
objective it is an anti-lever --- a qubit-lower-bound \emph{witness}, not a shippable circuit. Treat
sub-948q PZ numbers as combinatorial lower-bound claims. The takeaway generalizes: \textbf{a qubit
lever whose savings are funded by data-dependent recompute around every gate will violate break-even
by a large factor --- always price the recompute, not just the freed lanes.}
...
\subsection{The parallel low-qubit witness track (a separate line --- NOT the product SOTA)}

Running \emph{alongside} the product-SOTA journey above is a separate \textbf{Shrunken-PZ /
Proos--Zalka divstep} line whose only goal is to minimize qubits, ignoring score. It is worth
seeing because it maps the qubit \emph{floor} and demonstrates the exchange-rate trap
(\S\ref{sec:regshare}, \S\ref{sec:exchange}):

\begin{center}
\begin{tabular}{p{2.6cm}p{6.5cm}p{4cm}}
\toprule
\textbf{Qubit record} & \textbf{Technique} & \textbf{Status} \\
\midrule
1050q & trailmix shrunken-PZ embedded op stream & witness \\
1019q & source-level dynamic-width PZ port & witness \\
988$\to$956q & dirty-borrow / gate-hosting lever stack & witness \\
952$\to$948q & further lane borrowing + thin schedule & witness (lowest \emph{validated}) \\
\textbf{851q} (\texttt{e7dd3de}, 6/23) & \textbf{EEA register sharing (\S\ref{sec:regshare}), Luo 2026} & \textbf{analysis oracle --- rejected $+3659\%$} \\
829q (\texttt{1dd61ca}, 6/24) & more borrowing/relocation on a frozen envelope & analysis oracle --- rejected \\
\bottomrule
\end{tabular}
\end{center}

Everything below 948q is the \textbf{analysis-oracle regime}: it validates $0/0/0$ only on its own
island, costs $\sim$400--500M Toffoli ($\sim$250$\times$ over the product break-even), and the
implementing module explicitly disclaims being a reversible circuit. \textbf{851q and 829q are
qubit-lower-bound claims, not shippable circuits.} The product SOTA (1152q $\times$ 1.36M) lives in
a completely different regime: more qubits, but $\sim$340$\times$ fewer Toffoli, and value-exact.
The two minima are not reached by the same construction --- which is the whole lesson of
\S\ref{sec:regshare}.
...
EEA register sharing (\S6.O) & $\sim$97--119 (to 851$\to$829q) & \textbf{8.5$\times$ explosion} (anti-lever) & Yes (oracle, not a circuit) \\
\bottomrule
\end{tabular}
\end{center}
