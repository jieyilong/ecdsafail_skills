...
\begin{enumerate}
  ...
  \item \textbf{Explore the (Q, T) Pareto frontier} --- especially the \emph{useful corner} where
    \textbf{both} Q and T are lower than the published Google Quantum AI / Babbush--Gidney resource
    estimates for secp256k1 ($\leq$1200q / $\leq$90M Toffoli, arXiv:2603.28846). A point is
    Pareto-optimal if you cannot lower one of Q or T without raising the other. The goal here is not
    a single score but a \emph{clean, reusable basis curve}: a value-exact circuit at each qubit
    count that others can fork and improve (see the 1153q$\to$1133q clean Pareto bases,
    \S\ref{sec:history}). This track deliberately avoids island-overfit tricks (\S\ref{sec:density})
...
\midrule
Live-range hole (\S6.A) & remove a pebble by \textbf{uncompute}, re-place later by \textbf{recompute} & Bennett & full Toffoli, \emph{twice} \\
Free-and-recompute cy0 (\S6.E) & same, on one cheap pebble & Bennett & $\approx$0 (a few CNOTs) \\
MBU / carry vent (\S8.A--B) & remove a carry pebble by \textbf{X-measurement $+$ CZ} & spooky & \textbf{0 Toffoli} \\
Conditional replay (\S8.C) & measure the pebble; recompute only on the firing branch & spooky & $\Pr[\text{fire}] \times$ cost \\
HMR ghosting (\S6.N) & measure, leave a \textbf{ghost}, recompute $+$ phase-fix & spooky & one recompute \\
Transcript compression (\S6.H) & \textbf{don't pebble it} --- shrink the data so fewer pebbles persist & (out of the game) & codec only \\
...
hole (\S6.A) is a \emph{Bennett} move: to free a pebble you uncompute it (paying its Toffoli) and
later recompute it (paying again) --- the classical-reversible game, where un-pebbling is expensive.
MBU (\S8.A) is the \emph{spooky} move on the same kind of pebble: you remove it by \textbf{measuring}
...
\begin{itemize}
  \item \textbf{The transcript is firmly in the ``never recompute'' regime.} Bennett's
    ...
    the data} (\S6.H), not by recomputing it.
...
beats Google's secp256k1 estimate on \emph{both} axes ($q < 1200$, and $T \approx$ 1.37--1.46M is
$\sim$60$\times$ under the $\leq$90M Toffoli budget). They are ``rejected'' on the Q$\times$T product
\emph{by construction} (they trade qubits down for Toffoli up); the deliverable is the exchange
\emph{curve}, not a score.
...
qubits --- clamp a peak fold's measured-vent windows, \S8.B), \textbf{no-ancilla / dirty-borrow
...
\S6.A/\S\ref{sec:pebbling}), and --- new at 1133 --- \textbf{GCD active-width trimming}: per-step
shrinking of the live GCD operand width in the convergence tail
(\texttt{TLM\_GCD\_ACTIVE\_WIDTH\_TRIM=3} after step 205), the dynamic-width-register idea (\S6.F)
...
Reset-bounded id compaction (\S6.Q) & gap (max-id $-$ true peak) & 0 & Yes (relabels wires only) \\
GCD active-width trim (\S6.F applied) & varies (deep tail) & small & Partial (graded; nonce-hunted) \\
\bottomrule
\end{tabular}
\end{center}
...
Empirical dead-CCX drop (\S8.H) & $\approx13{,}000+$ (1st pass) & 0 & No (island-overfit) \\
Iterated 2-pass dead-CCX (\S8.H) & $\approx2{,}400$/extra pass & 0 & No (island-overfit) \\
\bottomrule
\end{tabular}
\end{center}
