...
- The `trailmix_ludicrous` module is now the **base to fork from**, not the dialog-GCD 1168 route. SOTA = `f8e215b`/`4966254`, **1159q × 1,378,242 = 1,597,382,478** (cheap Karatsuba arithmetic + the 1159q headroom clamp, then Toffoli-ground via `GAP_J2` comparator narrowing + converged-tail cswap elision + the doubling-ramp removal in the Karatsuba reduce).
