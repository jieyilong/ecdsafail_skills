...
- The `trailmix_ludicrous` module is now the **base to fork from**, not the dialog-GCD 1168 route. SOTA = `20b9a1d`/`19cf407`, **1159q × 1,364,380 = 1,581,316,420** (cheap Karatsuba arithmetic + the 1159q headroom clamp, Toffoli-ground via `GAP_J2` narrowing + converged-tail cswap elision + doubling-ramp removal + **empirical dead-CCX elimination**). The single most promising open axis is a **bigger/fresher dead-CCX screen** (§2.8) — peak-neutral, open-ended, additive on everything else; plus wiring on the scaffolded **recursive Karatsuba** (`TLM_SQUARE_KARA2`).
