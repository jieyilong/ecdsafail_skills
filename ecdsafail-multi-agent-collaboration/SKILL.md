...
description: Use this skill when coordinating two or more agents, such as Claude and Codex, on the ECDSA Fail challenge or another benchmark-driven circuit-optimization hunt. Trigger whenever the user mentions collaborating with another agent, a mailbox, Claude/Codex teamwork, exchanging hypotheses, independent cross-checks, splitting GPU/research work, or converging on a route to beat SOTA. This skill turns multi-agent chat into a disciplined scientific loop: read the thread, share exact evidence, divide experiments, reconcile disagreements, and preserve the winning route.
---
